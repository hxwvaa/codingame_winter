...
while True:
    ...
    def get_next_action():
        global used_organs, used_roots, double_check_T_B
        ...
        if best_growth:
            ...
            if x >= 0 and y >= 0 and x < width and y < height:
                if type_ == "BASIC" and my_a >= 1:
                    for o_x, o_y, o_id, _, _, _ in my_organs:
                        if organ_id == o_id:
                            cur_x, cur_y = o_x, o_y
                    if my_b >= 2 and my_c >= 2 and double_check_T_B == False:
                        double_check_T_B = True
                        used_roots.add(organ_root_id)
                        used_organs.add(organ_id)
                        occupied_positions.add((x, y))
                        direction = get_harvester_direction(cur_x, cur_y, x, y) #get direction of where it growing
                        return f"GROW {organ_id} {x} {y} TENTACLE {direction}"
                    else:
                        double_check_T_B = False
                        used_roots.add(organ_root_id)
                        used_organs.add(organ_id)
                        occupied_positions.add((x, y))
