...
def find_active_harvesters_with_protein_type(my_organs, proteins):
    """
    Find the harvesters that are actively harvesting proteins and return the type of protein being harvested.
    ...
    Returns:
        list: A list of tuples where each tuple contains:
              (harvester_id, protein_type).
    """
    active_harvesters = []

    # Convert proteins list to a dictionary for quick lookup of protein type by position
    protein_dict = {(px, py): protein_type for px, py, protein_type in proteins}
    ...
    for organ_x, organ_y, organ_id, organ_type, organ_dir, organ_root_id in my_organs:
        if organ_type == "HARVESTER" and organ_dir in direction_offsets:
            ...
            if target_position in protein_dict:
                protein_type = protein_dict[target_position]
                active_harvesters.append((organ_id, protein_type))

    return active_harvesters

def step_on_missing_protein(my_organs, proteins, my_a, my_b, my_c, my_d, used_roots):
    """
    Check if any organ should step on a protein of a type that is not being harvested.

    Args:
        my_organs (list): List of organs with positions, types, and directions.
        proteins (list): List of proteins with positions and types.
        my_a, my_b, my_c, my_d (int): The counts of protein types A, B, C, and D that have been harvested.
        used_roots (set): Set of root IDs that have been used.

    Returns:
        None: The function modifies the state of the organs as needed.
    """
    # Identify which protein types are not being harvested
    not_being_harvested = set()
    harvesting_types = find_active_harvesters_with_protein_type(my_organs, proteins)

    # Check which protein types are missing or not being harvested
    harvested_types = {protein_type for _, protein_type in harvesting_types}
    
    if my_a < 1 and 'A' not in harvested_types:
        not_being_harvested.add('A')
    if my_b < 1 and 'B' not in harvested_types:
        not_being_harvested.add('B')
    if my_c < 1 and 'C' not in harvested_types:
        not_being_harvested.add('C')
    if my_d < 1 and 'D' not in harvested_types:
        not_being_harvested.add('D')

    # For each organ, check if it's next to a protein that is not being harvested
    for organ_x, organ_y, organ_id, organ_type, organ_dir, organ_root_id in my_organs:
        if organ_root_id in used_roots:
            continue
        
        # Check neighboring positions for proteins of types that are not being harvested
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            x, y = organ_x + dx, organ_y + dy
            
            # Check if the neighbor is within bounds
            if 0 <= x < width and 0 <= y < height:
                # Check if the neighbor contains a protein
                for px, py, protein_type in proteins:
                    if (x, y) == (px, py) and protein_type in not_being_harvested:
                        # Step on the protein if it's not being harvested
                        if my_a >= 1:
                            used_roots.add(organ_root_id)
                            used_organs.add(organ_id)
...
def is_tentacle_next_to_coordinate(target_x, target_y, enemy_organs):
    """
    Check if there is a tentacle next to the given coordinate, facing that coordinate.

    Args:
        target_x (int): The x-coordinate where you want to check.
        target_y (int): The y-coordinate where you want to check.
        enemy_organs (list): List of enemy organs with their positions, types, and directions.
                             Each entry is a tuple (ex, ey, organ_type, organ_dir).
                             Where:
                                 - ex, ey are the coordinates of the enemy organ.
                                 - organ_type is the type of the organ (e.g., "TENTACLE").
                                 - organ_dir is the direction the organ is facing (e.g., "N", "S", "E", "W").

    Returns:
        bool: True if there is a tentacle next to the target coordinate and facing it, otherwise False.
    """
    # Define direction offsets
    direction_offsets = {
        "N": (0, -1),
...
while True:
    ...
    def get_next_action():
        ...
        # if my_a < 1 or my_b < 1 or my_c < 1 or my_d < 1:
        #     step = step_on_missing_protein(my_organs, proteins, my_a, my_b, my_c, my_d, used_roots)
        #     if step:
        #         return step
                    ...
                    if free_count >= 4:
                        ...
                        if not is_root_in_direction((sporer_x, sporer_y), sporer_dir, my_organs):
                            ...
                            if spore_result:
                                ...
                                if is_tentacle_next_to_coordinate(spore_pos[0], spore_pos[1], enemy_tentacles):
                                    continue
