...
def find_best_growth_position(my_organs, proteins, occupied_positions, used_roots, width, height):
    """
    Finds the best position to grow an organ, prioritizing empty positions near the organ before stepping on proteins.

    Args:
        my_organs (list): List of organs with their details.
        proteins (list): List of protein positions.
        occupied_positions (set): Set of occupied positions on the grid.
        used_roots (set): Set of root IDs that have already been used.
        width (int): Width of the grid.
        height (int): Height of the grid.

    Returns:
        tuple: Best growth position and details, or None if no valid position is found.
    """
    best_score = float('inf')
    best_growth = None
    ...
    for organ_x, organ_y, organ_id, organ_type, organ_dir, organ_root_id in my_organs:
        ...
        # Check for empty positions first
        valid_positions = [
            (new_x, new_y) for new_x, new_y in possible_positions
            if 0 <= new_x < width and 0 <= new_y < height and 
            (new_x, new_y) not in occupied_positions
...
while True:
    ...
    def get_next_action():
        ...
        if my_b >= 1 and my_d >= 1 and count_active_harvesters(my_organs, proteins) < 2:
