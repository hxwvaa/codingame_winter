...
def find_best_growth_position(my_organs, proteins, occupied_positions):
    ...
    # Track whether there are any empty positions available globally
    empty_positions_available = False
    ...
    for organ_x, organ_y, organ_id, organ_type, organ_dir in my_organs:
        ...
        if valid_positions:
            empty_positions_available = True

        # If empty positions are available, only consider them
        if empty_positions_available:
            valid_positions = [
                (new_x, new_y) for new_x, new_y in valid_positions
            ]
        else:  # If no empty positions, allow stepping on proteins
            valid_positions = [
                (new_x, new_y) for new_x, new_y in possible_positions
                if 0 <= new_x < width and 0 <= new_y < height and 
                (new_x, new_y) in {(p[0], p[1]) for p in proteins}
...
def direction_to_offset(direction):
    if direction == 'N':
        ...
        return (-1, 0)
    return (0, 0)  # Default for invalid direction
...
while True:
    ...
    def get_next_action():
                    ...
                    for ax, ay, dir_ in possible_attack_positions:
                        ...
                        if abs(ax - organ_x) + abs(ay - organ_y) == 1:
                            ...
                            if (ax, ay) not in occupied_positions and (ax, ay) not in enemy_occupied_positions:
                                # Check if the position is not directly in front of an enemy tentacle
                                if (ax, ay) not in [(etx + dx, ety + dy) for etx, ety, edir in enemy_tentacles
                                                    for dx, dy in direction_to_offset(edir)]:
                    ...
                    if abs(organ_x - ex) + abs(organ_y - ey) == 2:
                        ...
                        for px, py, _ in proteins:
                            if (px, py) == (mid_x, mid_y):
                                # Ensure not growing in front of an enemy tentacle
                                for etx, ety, edir in enemy_tentacles:
                                    dx, dy = direction_to_offset(edir)
                                    if (mid_x, mid_y) == (etx + dx, ety + dy):
                                        break  # Don't grow here
                                else:
        ...
        # Fifth priority: Expand to empty positions
